...
public class DuckRange extends JPanel {
	...
	public void newGame(){
		...
		if(twoDucks){
			for(int i = 0; i < 10/2; i++){
				ducks.add(2*i,new Duck(400,325,1,1));
				ducks.add((2*i)+1,new Duck(0,325,1,1));
			}
		...
		powerUpIndex = 0;
		for(int i = 0; i < 2; i++){
			powerUps.add(i,new SlowDown(200,0,10,10));
		}
	...
	public boolean hit(Point point, Duck duck){
		return (duck.x <= point.x && point.x <= (duck.x + 61) &&
				duck.y <= point.y && point.y <= (duck.x + 72));
	}
